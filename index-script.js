...
function get(){
    ...
    setTimeout(()=>{
        axios.get("https://cv-v3-api-2j3j.vercel.app/api/get")
        .then(res => {
            myData = res.data;
...
function save(){
    axios.post("https://cv-v3-api-2j3j.vercel.app/api/set", myData)
    .then(res => {
        get();
    })
}
...
function getSkill(skill){
    if(skill._id === null){
        ...
        return `
            <div id="x${skillEditId}" data-id="${skill._id}" class="user-inputbox">
            <label for="skill"><i class="fas fa-skill contact-icon"></i>  Skill <button onclick="removeSkills('x${skillEditId}')">-</button></label>
            <input onkeyup="keyupSkill(event, 'title')" type="text" id="skillTitle${skillEditId}" data-id="${skill._id}" name="skill" value="${skill.title}" placeholder="Enter your skill"/>
            </div> `
    }
}
...
function addSkill(){
    skillEditId++;
    const skill = {_id: null, id:skillEditId, title:"", logo:""}
...
function getMedias(media){
    if(media._id === null){
        ...
        return `
            <div id="link${mediaEditId}" data-id="${media._id}">
            <label for="link"><i class="fas fa-link contact-icon"></i> Link </label>
            <input onkeyup="keyupMedias(event, 'link')" type="text" id="link${mediaEditId}" data-id="${media._id}" value="${media.link}" name="link" placeholder="Enter your link"/><button onclick="removeMedias('link${mediaEditId}')">-</button>
            </div>
            `
...
function addMedias(){
    mediaEditId++;
    const media = {_id: null, id:mediaEditId, title:"", link:"",icon:""}
...
function keyupMedias(event, name){
    const element = event.target;
    const id = element.dataset["id"];
    const index = myData.medias.findIndex(p=> p.id == id || p._id == id);
    // if(name =="title"){
    //     myData.skill[index].title = lement.value;
    // }
    // else if(name == "logo"){
...
function getExperiences(experience){
    if(experience._id === null){
        ...
        return `
            <div id="y${experienceEditId}" data-id="${experience._id}" class="contact">
                <div id="addCompany" class="user-inputbox">
                    <label for="company"> Company </label>
                    <input onkeyup="keyupExperiences(event, 'subTitle')" type="text" id="company${experienceEditId}" value="${experience.subTitle}" data-id="${experience._id}" name="company" placeholder="Enter Company"/>
                </div>
            
                <div id="addPosition" class="user-inputbox">
                    <label for="position"> Position </label>
                    <input onkeyup="keyupExperiences(event, 'title')" type="text" id="position${experienceEditId}" value="${experience.title}" data-id="${experience._id}" name="position" placeholder="Enter your Position"/>
                </div>
                            
                <div id="addDate" class="user-inputbox">
                    <label for="date"> Date </label>
                    <input onkeyup="keyupExperiences(event, 'date')" type="text" id="date${experienceEditId}" value="${experience.date}" data-id="${experience._id}" name="date" placeholder="Enter Date"/>
                </div>
    
                <div id="addDescription" class="user-inputbox">
                    <label for="description"> Description </label>
                    <input onkeyup="keyupExperiences(event, 'description')" type="text" id="description${experienceEditId}" value="${experience.description}" data-id="${experience._id}" name="description" placeholder="Enter Description"/>
                </div>
                <button onclick="removeExperiences('y${experienceEditId}')">-</button>
            </div> `
    }
...
function addExperiences(){
    experienceEditId++;
    const experience = {_id: null, id:experienceEditId, title:"", subTitle:"", date:"",description:""}
...
function getEducations(education){
    if(education._id === null){
        ...
        return `
            <div id="z${educationEditId}" data-id="${education._id}" class="contact">
                <div id="addSection" class="user-inputbox">
                    <label for="section"> Section </label>
                    <input onkeyup="keyupEducations(event, 'title')" type="text" id="section${educationEditId}" data-id="${education._id}" value="${education.title}" name="section" placeholder="Enter your section"/>
                </div>
            
                <div id="addUniversity" class="user-inputbox">
                    <label for="university"> University </label>
                    <input onkeyup="keyupEducations(event, 'faculty')" type="text" id="university${educationEditId}" data-id="${education._id}" value="${education.faculty}" name="university" placeholder="Enter your university"/>
                </div>
        
                <div id="addDegree" class="user-inputbox">
                    <label for="degree"> Degree </label>
                    <input onkeyup="keyupEducations(event, 'degree')" type="text" id="degree${educationEditId}" data-id="${education._id}" value="${education.degree}" name="degree" placeholder="Enter your degree"/>
                </div>
                            
                <div id="addDate" class="user-inputbox">
                    <label for="date"> Date </label>
                    <input onkeyup="keyupEducations(event, 'date')" type="text" id="date${educationEditId}" data-id="${education._id}" value="${education.date}" name="date" placeholder="Enter Date"/>
...
function addEducations(){
    educationEditId++;
    const education = {_id: null, id:educationEditId, title:"", faculty:"", degree:"", date:""}
...
function getReferances(referance){
    if(referance._id === null){
        ...
        return `
            <div id="ref-div${referanceEditId}" data-id="${referance._id}" class="user-info">
                <div class="user-inputbox">
                    <label for="username">Full Name</label>
                    <input onkeyup="keyupReferances(event, 'name')" type="text" id="name-text${referanceEditId}" data-id="${referance._id}" value="${referance.name}" name="name-text" placeholder="Enter your referance name"/>
                </div>
            
                <div class="user-inputbox">
                    <label for="title">Title</label>
                    <input onkeyup="keyupReferances(event, 'title')" type="text" id="title-text${referanceEditId}" data-id="${referance._id}" value="${referance.title}" name="title-text" placeholder="Enter your referance title"/>
                </div>
            
                <div id="addphone" class="user-inputbox">
                    <label for="phone"><i class="fas fa-phone contact-icon"></i>  Phone </label>
                    <input onkeyup="keyupReferances(event, 'phone')" type="tel" id="tel${referanceEditId}" data-id="${referance._id}" value="${referance.phone}" name="tel" placeholder="Enter your referance phone number"/>
                </div>
            
                <div id="addemail" class="user-inputbox">
                    <label for="email"><i class="fas fa-envelope contact-icon"></i> Email </label>
                    <input onkeyup="keyupReferances(event, 'email')" type="text" id="email${referanceEditId}" data-id="${referance._id}" value="${referance.email}" name="email" placeholder="Enter your referance email"/>
                </div>
                <button onclick="removeReferances('ref-div${referanceEditId}')">-</button>
            </div>`
    }
...
function addReferances(){
    referanceEditId++;
    const referance = {_id: null, id:referanceEditId, name:"", title:"", subTitle:"", phone:"", phoneIcon:"", email:"", emailIcon:""}
...
function keyupReferances(event, name){
    const element = event.target;
    const id = element.dataset["id"];
    const index = myData.referances.findIndex(p=> p.id == id || p._id == id);
    // if(name =="title"){
    //     myData.skill[index].title = lement.value;
    // }
    // else if(name == "logo"){
